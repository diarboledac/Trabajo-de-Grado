...
## 4. Ejecución local
### 4.0 Dashboard interactivo (solo visualizaci�n)
- Lanza el servidor Flask de m�tricas (puerto por defecto 5050):
  ```bash
  py -3 - <<'PY'
  ...
  PY
  ```
- Abre `http://<host>:5050`: ver�s las gr�ficas de m�tricas agregadas. Si el puerto est� ocupado, ajusta `port` al crear `MetricsServer`.
...
## 11. Documento de grado (IEEE)
- El manuscrito formal vive en `doc/DocumentoGrado.tex` junto con `doc/refs.bib`. Sigue el formato IEEEtran (conference) y cita todas las fuentes mediante `biblatex` + `biber`.
- El documento incorpora las mismas métricas/tablas reales que se registran en `data/metrics/` y describe los scripts del repositorio. Cuando se añadan nuevas pruebas, actualiza tanto las tablas (por ejemplo, la de la Sección 6) como las referencias cruzadas.
- Para compilar localmente (con TeX Live): `latexmk -pdf -interaction=nonstopmode doc/DocumentoGrado.tex && biber DocumentoGrado && latexmk -pdf -interaction=nonstopmode doc/DocumentoGrado.tex`. En Overleaf selecciona motor pdfLaTeX y backend Biber.
- Los datos personales del autor están marcados como espacios editables en el bloque `\author{...}`; remplázalos antes de entregar la versión final.
- Si agregas figuras, guárdalas en `doc/figs/` y referencia con `\includegraphics{figs/<nombre>.pdf}`; recuerda actualizar tanto README como DocumentoGrado para describir cualquier nuevo artefacto.
