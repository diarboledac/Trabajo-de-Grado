...
scripts/
  DISPOSITIVOS MQTT/        # MQTT tooling (simulators, provisioning, helpers)
    ...
    ...
  DISPOSITIVOS LwM2M/       # Placeholder for LwM2M tooling (coming soon)
requirements.txt
.env                        # local-only configuration (never commit secrets)
```
Everything under `data/` is ignored by Git so tokens and logs stay local. All MQTT-related scripts now reside inside `scripts/DISPOSITIVOS MQTT/`; LwM2M tooling will live next door.
...
```bash
py -3 "scripts/DISPOSITIVOS MQTT/create_devices.py"
```
What happens:
...
```bash
py -3 "scripts/DISPOSITIVOS MQTT/mqtt_stress_async.py"
```
This single command:
...
- `MetricsCollector` (in `scripts/DISPOSITIVOS MQTT/send_telemetry.py`) aggregates metrics under thread locks.
- `MetricsServer` (in `scripts/DISPOSITIVOS MQTT/metrics_server.py`) exposes the `/api/metrics` endpoint consumed by the dashboard and any external tooling.
...
|--------|-------------|----------------|
| `scripts/DISPOSITIVOS MQTT/create_devices.py` | Provisions/updates the simulated fleet; saves tokens and CSV. | Run whenever the device fleet needs to change or tokens should refresh. |
| `scripts/DISPOSITIVOS MQTT/mqtt_stress_async.py` | Async orchestrator, shard manager, and global dashboard bootstrapper. | Start the load test; stop with `Ctrl+C`. |
| `scripts/DISPOSITIVOS MQTT/metrics_server.py` | Flask server used internally by the orchestrator (aggregates all shards). | Imported automatically; seldom run standalone. |
| `scripts/DISPOSITIVOS MQTT/report_last_run.py` | Prints a friendly summary of the latest run’s JSON report. | Use after each run to capture KPIs. |
| `scripts/DISPOSITIVOS MQTT/check_connectivity.py` | Simple REST smoke test (login + one-page device list). | Run before provisioning if unsure about network/credentials. |
| `scripts/DISPOSITIVOS MQTT/delete_devices.py` | Deletes devices listed in `data/provisioning/devices.csv`. | Cleanup after lab sessions. |
| `scripts/DISPOSITIVOS MQTT/delete_by_prefix.py` | Deletes all tenant devices whose name starts with `DEVICE_PREFIX`. | Broad cleanup tool (use carefully). |
| `scripts/DISPOSITIVOS MQTT/send_telemetry.py` | Legacy threaded simulator maintained for comparison experiments. | Optional; new tests should use `scripts/DISPOSITIVOS MQTT/mqtt_stress_async.py`. |
| `scripts/DISPOSITIVOS MQTT/tb.py` | Lightweight ThingsBoard REST client (shared by other scripts). | Internal helper; review if extending functionality. |
...
|----------|--------|-------|
| Different fleet size | Update `DEVICE_COUNT` (and optionally `DEVICE_PREFIX`). Re-run `scripts/DISPOSITIVOS MQTT/create_devices.py`. | `.env`, `scripts/DISPOSITIVOS MQTT/create_devices.py` (auto reads `.env`). |
| Faster/slower publish rate | Modify `PUBLISH_INTERVAL_SEC`. Restart the simulator. | `.env`. |
| TLS-enabled MQTT broker | Set `MQTT_TLS=1` and ensure certificates are trusted (see `paho-mqtt` docs). | `.env`, optionally extend `scripts/DISPOSITIVOS MQTT/send_telemetry.py`. |
| Additional telemetry fields | Edit the `payload()` method in `scripts/DISPOSITIVOS MQTT/send_telemetry.py`. Maintain JSON-compatible values. | `scripts/DISPOSITIVOS MQTT/send_telemetry.py`. |
| Alternate dashboard port | Change `METRICS_PORT` in `.env`. | `.env`. |
| Export metrics to another system | Consume `/api/metrics` or extend `MetricsServer` to push data elsewhere (Prometheus, InfluxDB, etc.). | `scripts/DISPOSITIVOS MQTT/metrics_server.py`. |
...
- **`tokens.json` missing** - Run `py -3 "scripts/DISPOSITIVOS MQTT/create_devices.py"` first.
...
5. Launch the simulator (`scripts/DISPOSITIVOS MQTT/mqtt_stress_async.py`) and monitor the global dashboard (default http://localhost:5050).
